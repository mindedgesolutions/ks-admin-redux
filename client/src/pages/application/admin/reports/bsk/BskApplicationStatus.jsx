...
const BskApplicationStatus = () => {
  document.title = `B.S.K Application Status Report | ${
    import.meta.env.VITE_ADMIN_TITLE
  }`;

  // Report based static data starts ------
  const startDate = new Date("09-14-2023");
  const endDate = new Date();
  const allDataApi = `/reports/bsk-application-status-all`;
  const breadCrumb = <Link to="/admin/reports">Back to Reports</Link>;
  const resetUrl = `/admin/reports/bsk/application-status`;
    ...
    if (queryParams.get("dist")) {
      ...
      try {
        const response = await customFetch.get(
          `/reports/bsk-application-status`,
          {
            params: {
  ...
  return (
    <>
      <UserPageHeader
        title={`B.S.K Application Status Report`}
...
export default BskApplicationStatus;
