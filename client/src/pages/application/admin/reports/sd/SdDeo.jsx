...
const SdDeo = () => {
  document.title = `Special Drive : DEO Report | ${
  ...
  const resetUrl = `/admin/reports/sd/deo`;
  ...
  const gotoDeoList = (filterDist, filterSubdiv, filterBlock, count) => {
    ...
    const dist = districts.find((c) => c.district_code === filterDist)
      .district_name;
    const subdiv = subdivs?.find((c) => c.subdiv_code === selectSubdiv)
      ?.subdiv_name;
    const block = blocks.find((c) => c.block_mun_code === selectBlock)
      ?.block_mun_name;
    ...
    setIsLoading(false);
    navigate(`/admin/reports/sd/deo-list`);
  };

  return (
    <>
      <UserPageHeader
        title={`Special Drive : DEO Report`}
